...
def frontlever(cap, cooldown, last_spoken_time, previous_pose, mp_drawing, mp_pose, start_time, correct_pose_start_time, total_correct_time):
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ret, frame = cap.read()
            if not ret:
            ...
            if results.pose_landmarks:
                ...
                elif not 160 < head_angle_l < 185 and not 160 < head_angle_r < 185:
                ...
                all_angles = [hand_angle_l, back_angle_l, head_angle_l, shoulder_angle_l, leg_angle_l, hand_angle_r, head_angle_r, back_angle_r, shoulder_angle_r, leg_angle_r]
                ideal_angles = [180, 180, 180, 45, 180, 180, 180, 180, 45, 180]
                errors = [abs(a - b) for a, b in zip(ideal_angles, all_angles)]
                accuracies = [max(0, 100 - (e / 30) * 100) for e in errors]
                avg_accuracy = sum(accuracies) / len(accuracies)
                ...
                elapsed_time = int(time.time() - start_time)
                correct_time_display = int(total_correct_time) if correct_pose_start_time is None else int(
                    total_correct_time + (time.time() - correct_pose_start_time))
    ...
    print(f"Total time: {int(time.time() - start_time)} seconds")
    print(
        f"Total time with correct pose: {int(total_correct_time) if correct_pose_start_time is None else int(total_correct_time + (time.time() - correct_pose_start_time))} seconds")
...
while True:
    ...
    if n == "lsit":
        ...
        frontlever(cap, cooldown, last_spoken_time, previous_pose, mp_drawing, mp_pose, start_time, correct_pose_start_time, total_correct_time)
    else:
        break
# ---------------------------------------------------------------------------------------------------
