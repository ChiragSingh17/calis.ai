...
def lsit(cap, cooldown, last_spoken_time, engine, speak_lock, previous_pose, mp_drawing, mp_pose):
...
def handstand(cap, cooldown, last_spoken_time, engine, speak_lock, previous_pose, mp_drawing, mp_pose):
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ret, frame = cap.read()
            if not ret:
...
def frontlever(cap, cooldown, last_spoken_time, engine, speak_lock, previous_pose, mp_drawing, mp_pose):
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ret, frame = cap.read()
            if not ret:
...
while True:
    ...
    n.lower()
    if n == "lsit":
        lsit(cap, cooldown, last_spoken_time, engine, speak_lock, previous_pose, mp_drawing, mp_pose)
    elif n == "handstand":
        handstand(cap, cooldown, last_spoken_time, engine, speak_lock, previous_pose, mp_drawing, mp_pose)
    elif n == "frontlever":
        frontlever(cap, cooldown, last_spoken_time, engine, speak_lock, previous_pose, mp_drawing, mp_pose)
    else:
        break
# ---------------------------------------------------------------------------------------------------
